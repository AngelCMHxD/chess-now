...
const sendRequest = async () => {
	...
	errorMsg.value = "";
	try {
		await chessClient.sendFriendRequest(usernameStr);
		emit("sent");
		emit("close");
	} catch (e: unknown) {
		errorMsg.value =
			(e as { message?: string })?.message || "Failed to send request";
	} finally {
		loading.value = false;
	}
};
...
<template>
	<Box
		...
		:width="40"
	>
		<Text :content="t`${bold('Send Friend Request')}`" />
		<Text :content="'Enter username:'" :marginTop="1" />
